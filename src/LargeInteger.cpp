...
// should go here
LargeInteger::LargeInteger(int size, int newArray[])
{
  id = nextLargeIntegerId++;
  numDigits = size;
  digit = new int[numDigits];
...
void LargeInteger::appendDigit(int append)
{
  if (append > 0)
  {
    int newNum = numDigits + 1;
    int* ptrarray;
    ptrarray = new int[newNum];
    for (int i=0; i<numDigits; i++)
    {
      ptrarray[i] = digit[i];
    }
...
LargeInteger& LargeInteger::add(const LargeInteger& other)
{
  int* ptrArray;
  ...
  {
    int totalSum = (digitAtPosition(i) + other.digitAtPosition(i));
    totalSum += carryOn;
    int add = totalSum % 10;
    ptrArray[i] = add;
    if (totalSum > 9)
  ...
  LargeInteger* newDigit = new LargeInteger(size, ptrArray);
  (*newDigit).appendDigit(carryOn);
  delete[] ptrArray;
  return *newDigit;
}